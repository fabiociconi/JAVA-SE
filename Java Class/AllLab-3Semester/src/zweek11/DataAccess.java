...
public class DataAccess {
	private PreparedStatement findStudentStmt, updateStudentStmt, 
                addStudentStmt, findCourseStmt, addCourseStmt;
	private Connection conn;
	private ResultSet resultset;
        private String user ="system";
        private String pass ="cifa410829";
        

	DataAccess() throws Exception {
		// for uid, pass, put your uid and pass
		conn = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:XE", user,
				pass);
		findStudentStmt = conn.prepareStatement("SELECT * FROM player WHERE rtrim(studentid) = ? ");
		updateStudentStmt = conn.prepareStatement("UPDATE student SET student.first_name = ?, student.last_name = ? WHERE rtrim(student.studentid) = ?");
		addStudentStmt = conn.prepareStatement("INSERT into student values (?, ?, ?)");
		findCourseStmt = conn.prepareStatement("SELECT course_code from student_course where rtrim(studentid) = ?");
	...
	void addStudent(Student st) throws Exception {
		...
		addStudentCourses(st.getId(), st.getCourses());
                System.out.println("Insert student ok");
	}
	...
	private void addStudentCourses(String id, String courses[]) throws Exception{
		...
		for (String c: courses){
			...
			addCourseStmt.executeUpdate();
                        System.out.println("Insert course ok");
		}
	}
	...
	public static void main(String s[]) throws Exception {
		...
		try {
			da = new DataAccess();
			Student st = da.findStudent("100");
			System.out.println("studentid="+st.getId());
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		}finally{
